            ...
            <?php if(!empty($news[0]['photo']['path']) || (!empty($settings['no_photo']) && !empty($settings['no_photo']['path']))): ?>
                ...
                <figure>
                    <a <?php if(!empty($news[0]['link_redirect'])): ?> onclick="CountNews(<?=$news[0]['id'];?>);"<?php endif; ?> href="<?php if(!$news[0]['link_redirect']): ?>/<?php endif; ?><?=$news[0]['link'];?>" title="<?=esc($news[0]['title']);?>"<?php if(!empty($news[0]['link_external'])): ?> target="_blank"<?php endif; ?>>
                        <img src="/image/c/880/525/<?=!empty($news[0]['photo']['path']) ? $news[0]['photo']['path'] : $settings['no_photo']['path']; ?>" alt="<?=esc($news[0]['title']);?> <?=esc($news[0]['photo']['caption']);?> <?=esc($news[0]['photo']['author']);?>">
                    </a>
                </figure>	
            <?php endif; ?>
            <div class="box">
                            ...
                            <?php if(!empty($item['photo']['path']) || (!empty($settings['no_photo']) && !empty($settings['no_photo']['path']))): ?>
                                <?php 
                                    if(!empty($item['photo']['path']) && !empty($item['photo']['crop_dimension'])) {
                                        if(!is_array($item['photo']['crop_dimension'])) $item['photo']['crop_dimension'] = json_decode($item['photo']['crop_dimension'], true); 
                                ...
                                <figure>
                                    <a <?php if(!empty($item['link_redirect'])): ?> onclick="CountNews(<?=$item['id'];?>);"<?php endif; ?> href="<?php if(!$item['link_redirect']): ?>/<?php endif; ?><?=$item['link'];?>" title="<?=esc($item['title']);?>"<?php if(!empty($item['link_external'])): ?> target="_blank"<?php endif; ?>>
                                        <img src="/image/c/450/328/<?=!empty($item['photo']['path']) ? $item['photo']['path'] : $settings['no_photo']['path']; ?>" alt="<?=esc($item['title']);?> <?=esc($item['photo']['caption']);?> <?=esc($item['photo']['author']);?>">
                                    </a>
                                </figure>	
                            <?php endif; ?>
                            <div class="caption">  
                        ...
                        <?php if($k<13 && $k>2):?>
                            <div class="item">
                                <?php if(!empty($item['photo']['path']) || (!empty($settings['no_photo']) && !empty($settings['no_photo']['path']))): ?>
                                    ...
                                    <figure>
                                        <a <?php if(!empty($item['link_redirect'])): ?> onclick="CountNews(<?=$item['id'];?>);"<?php endif; ?> href="<?php if(!$item['link_redirect']): ?>/<?php endif; ?><?=$item['link'];?>" title="<?=esc($item['title']);?>"<?php if(!empty($item['link_external'])): ?> target="_blank"<?php endif; ?>>
                                            <img src="/image/c/450/270/<?=!empty($item['photo']['path']) ? $item['photo']['path'] : $settings['no_photo']['path']; ?>" alt="<?=esc($item['title']);?> <?=esc($item['photo']['caption']);?> <?=esc($item['photo']['author']);?>">
                                        </a>
                                    </figure>	
                                <?php endif; ?>
                                <?php if(!empty($item['header'])):?>
