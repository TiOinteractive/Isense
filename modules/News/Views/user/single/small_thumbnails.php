<?php
/* 
Małe zdjęcie - galeria miniaturki
*/
?>
...
<section class="news-single news-<?=$data['id'];?> small-thumbnails">
    <div class="container">
        <?php if(!empty($data['subtitle']) || !empty($data['title'])): ?>
            ...
            <?php if(!empty($data['date']) || !empty($data['author'])): ?>
                <p class="date-author"><?=!empty($data['date']) ? lang('User.entertainment.Added') . ': ' . '<span>' . date('d.m.Y', strtotime($data['date'])) . ' ' . lang('User.entertainment.YearShort') . '</span>' : ''; ?><?=!empty($data['date']) && !empty($data['author']) ? ' / ' : ''; ?><?=!empty($data['author']) ? '<span>' . $data['author'] . '</span>' : ''; ?></p>
            <?php endif; ?>
        <?php endif; ?>
                        ...
                        <div class="photo">
                            <a href="/image/r/1920/1080/<?=$data['photo_big']; ?>"  data-thumb="/image/c/120/70/<?=$data['photo']['path'];?>" title="<?=esc($data['photo']['caption'] ? $data['photo']['caption']: $data['title']); ?>" rel="lightbox">
                                <picture>
                                    ...
                                    <img src="/image/<?=$data['photo_method'];?>/900/590/<?=$data['photo']['path']; ?>" alt="<?=esc($data['photo']['caption']); ?>" />
                                </picture>
                            </a>
                                ...
                                <?php if($p < 3): ?>
                                    <div class="thumbnail">
                                        <a href="/image/r/1920/1080/<?=$photo['path']; ?>"  data-thumb="/image/c/120/70/<?=$photo['path'];?>" title="<?=esc($photo['caption'] ? $photo['caption']: $data['title']); ?>" rel="lightbox">
                                            <picture>
                                                <img src="/image/c/400/240/<?=$photo['path']; ?>" alt="<?=esc($photo['caption']); ?>" />
                                            </picture>
