<?php
/* 
Najbliższe imprezy - strona główna -  wersja 1
 */
?>
                            ...
                            <div class="info">
                                <div class="date">
                                    <?php if(empty($event['date_end'])): ?>
                                        <strong><?=date('d', strtotime($event['date_order'])); ?></strong>
                                        <span><?=lang('Admin.months_short_names.' . date('F', strtotime($event['date_order']))); ?></span>
                                    <?php elseif($event['date_order'] > date('Y-m-d')): ?>
                                        <strong><?=date('d', strtotime($event['date_order'])); ?></strong>
                                        <span><?=lang('Admin.months_short_names.' . date('F', strtotime($event['date_order']))); ?></span>
                                    <?php else: ?>
