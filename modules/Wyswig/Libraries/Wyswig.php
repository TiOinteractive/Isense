...
class Wyswig
{
    ...
    public function index($content, $id_lang, $slug='')
    {
        $wyswig = $this->wyswigModel
                ...
                ->join('files fi', 'fi.id=wyswig.id_image', 'left')
                ->select('wyswig.id,wl.content,f.path as photo_path,fi.path as image_path,fi.name as image_name')
                ->where('wyswig.id_page_cont', $content['id'])
                ->where('wl.id_lang', $id_lang)
                ->first();
        // Grafiki — wspólne dla wszystkich wersji językowych
        if (!empty($wyswig)) {
            $wyswig['bg_image'] = !empty($wyswig['photo_path']) ? '/image/' . $wyswig['photo_path'] : '';
            $wyswig['image'] = !empty($wyswig['image_path']) ? '/image/' . $wyswig['image_path'] : '';
        }
        return $wyswig;
    }
