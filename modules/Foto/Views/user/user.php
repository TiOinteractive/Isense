<div id="user_photo">
    <h2 class="nag"><?= lang('Foto.UserGallery'); ?>: <?php if (!empty($data['nick'])): ?><?= $data['nick']; ?><?php elseif (!empty($data['name']) and!empty($data['surname'])): ?><?= $data['name']; ?> <?= $data['surname']; ?><?php endif; ?></h2>
    <h3 class="nag">Autor: <span><b><?= $data['nick']; ?></b></span> | Ilość prac: <span><b><?= ($data['cnt']['gallery']['gallery_cnt_photo'] + $data['cnt']['files']); ?></b></span></h3>

    <div id="category_single"> 
        <?php if (!empty($data['cnt']['gallery_count']) && $data['cnt']['gallery_count'] > 0 && !empty($data['gallery'])): ?>
            <h4><?= lang('Foto.Gallery'); ?> (<?= $data['cnt']['gallery_count']; ?>)</h4>
            <div class="list">
                            ...
                            <?php if ($gal['photo']): ?>
                                <picture>    
                                    <a href="/<?= $gal['link']; ?>" title="<?= esc($gal['name']); ?>">
                                        ...
                                        <img src="/image/c/740/490/<?= $gal['photo']; ?>" alt="<?= esc($gal['name']); ?>" class="trans400" />
                                    </a>	
                                </picture>
                                <div class="info">
            ...
            <?= $data['gallery_pager']; ?>
        <?php endif; ?>
        <?php if (!empty($data['cnt']['files']) and $data['cnt']['files'] > 0 and!empty($data['photos'])): ?>
            <h4><?= lang('Foto.Photos'); ?> (<?= $data['cnt']['files']; ?>)</h4>
            <div class="list">
                <?php foreach ($data['photos'] as $gal): ?>  
