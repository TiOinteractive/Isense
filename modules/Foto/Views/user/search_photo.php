<div id="search_photo">
    <h2 class="nag"><?= lang('Foto.SearchResult'); ?></h2>
    <h4 style="margin-top:0px;"><?= lang('Foto.SearchFound'); ?>: <span><?= $data['cnt_galleries']; ?></span> <?= lang('Foto.searchgallery'); ?> , <span><?= $data['cnt_photos']; ?></span> <?= lang('Foto.searchphoto'); ?> <?= lang('Foto.searchforword'); ?>: <span><?= $data['search_term']; ?></span></h4>
                                ...
                                <div class="info">
                                    ...
                                    <div class="photo_count">
                                        <?php if (!empty($gal['user_link'])): ?>
                                            <div><?= lang('Foto.photo'); ?>.: <a href="/<?= $gal['user_link']; ?>" title="<?php if(!empty($gal['nick'])):?><?= esc($gal['nick']); ?><?php elseif(!empty($gal['user_name']) and !empty($gal['user_surname'])):?><?= esc($gal['user_name']); ?> <?= esc($gal['user_surname']); ?><?php endif;?>"><b><?php if (!empty($gal['nick'])): ?><?= $gal['nick']; ?><?php elseif (!empty($gal['user_name']) && !empty($gal['user_surname'])): ?><?= $gal['user_name']; ?> <?= $gal['user_surname']; ?><?php endif; ?></b></a></div>
                                        <?php endif; ?> 
