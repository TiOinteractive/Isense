<form class="filters" action="<?=$locale ? '/' . $locale : ''; ?>/<?=env('ADMIN_PANEL_SLUG'); ?>/page/content/<?=$id_page; ?>/<?=$id_content; ?>" method="get">
	...
	<div class="filter">
        <label><?=lang('Foto.Investments'); ?></label>
        <select name="invest">
            <option value=""><?=lang('News.All'); ?></option>
            <option value="1"<?=isset($filters['invest']) && $filters['invest'] == 1 ? ' selected="selected"' : ''; ?>><?=lang('News.OnlyHome'); ?></option>
            <option value="0"<?=isset($filters['invest']) && $filters['invest'] == 0 ? ' selected="selected"' : ''; ?>><?=lang('News.OnlyNotHome'); ?></option>
        </select>
    </div>
    <div class="filter">
