<div class="form-row">
    <div class="form-label">
        <label><?=lang('Gallery.GalleriesList');?></label>
    </div>
            ...
            <?php if(!empty($lists)): ?>
                <?php foreach($lists as $list): ?>
                    <div class="form-col col-10">
                        <input type="checkbox" name="config[lists][]" value="<?=$list['id']; ?>" id="config-lists-<?=$list['id']; ?>" <?php if(!empty($page_content['config']) && !empty($page_content['config']['lists']) && in_array($list['id'], $page_content['config']['lists'])):?> checked="checked"<?php endif; ?> />
                        <label for="config-lists-<?=$list['id']; ?>"><?=$list['name']; ?></label>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
