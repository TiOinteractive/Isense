...
$d = $data ?? [];
$items = $d['items'] ?? [];
/* Kazda nowa linia pola "lead" = osobny akapit. */
$leadParas = array_values(array_filter(array_map('trim', preg_split('/\r\n|\r|\n/', (string) ($d['lead'] ?? ''))), 'strlen'));
?>
<section class="bg-[#F5F5F7] py-10 lg:py-14">
    <div class="max-w-[1300px] mx-auto px-4 lg:px-12">
        ...
        <?php if (!empty($leadParas)): ?>
            <div class="text-[#6E6E73] space-y-2 mb-10">
                <?php foreach ($leadParas as $para): ?><p><?= esc($para) ?></p><?php endforeach; ?>
            </div>
        <?php endif; ?>
