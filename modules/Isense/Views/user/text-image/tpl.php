...
$paras = preg_split('/\n\s*\n/', trim($d['body'] ?? ''));
$img = '<div class="rounded-3xl overflow-hidden' . ($side === 'left' ? ' order-1' : ' order-2') . '"><img src="' . esc($d['image'] ?? '', 'attr') . '" alt="' . esc($d['heading'] ?? '', 'attr') . '" class="w-full h-full object-cover"></div>';
ob_start(); ?>
