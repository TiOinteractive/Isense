<?php helper(['url', 'isense']); $boxes = $data['boxes'] ?? []; ?>
<section class="relative z-10 -mt-14 pb-0">
    <div class="max-w-[1300px] mx-auto px-4 lg:px-12">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <?php foreach ($boxes as $box): ?>
                <a href="<?= esc($box['link'] ?? '#', 'attr') ?>" class="group flex flex-col bg-white border border-[#E5E5EA] hover:border-[#3b81f7] rounded-sm p-7 shadow-xl transition-all hover:-translate-y-1">
                    <div class="flex justify-center mb-5"><?= isense_icon($box['icon'] ?? 'clipboard-list', 'w-9 h-9 text-[#3b81f7]', ['stroke-width' => '1.5']) ?></div>
                    ...
                    <div class="flex items-center justify-center gap-2 text-[#3b81f7] font-medium text-sm group-hover:gap-4 transition-all mt-auto">
                        <?= esc($box['cta'] ?? '') ?><?= isense_icon('arrow-right', 'w-4 h-4') ?>
                    </div>
                </a>
            <?php endforeach; ?>
        </div>
