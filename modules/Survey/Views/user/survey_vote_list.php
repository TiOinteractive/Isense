<?php if(!empty($results)):?>
	...
	<?php endif;?>
   <?=view('Modules\Survey\Views\user\survey_results_list', array('data' => $results['results'],'id'=>$survey['id']));?>   				  
<?php else:?>
<?php if(empty($votes)):?>
 ...
 <?php if(!empty($survey['options'])):?>
	  <?php foreach($survey['options'] as $option):?>
	    <div class="option <?php if(!empty($survey['single'])):?>single<?php endif;?><?php if(empty($votes)):?> error<?php endif;?>" id="option_<?=$option['id'];?>">
		   <input id="lopt<?=$option['id'];?>" name="option[]" type="<?php if(!empty($survey['single'])):?>radio<?php else:?>checkbox<?php endif;?>" value="<?=$option['id'];?>">
