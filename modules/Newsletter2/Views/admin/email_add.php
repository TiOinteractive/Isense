<div class="main-cont">
    ...
    <div class="c">
        <div class="head">
            <?php if(!empty($slider)): ?>
                <?=lang('Newsletter.EmailEdit'); ?>
            <?php else: ?>
                <?=lang('Newsletter.NewEmailAdd'); ?>
            <?php endif; ?>
        </div>
        <?= view('admin/alert_box', array('flashdata'=>!empty($flashdata) ? $flashdata : array())); ?>
        <form class="form slider-form" action="<?=$locale ? '/' . $locale : ''; ?>/<?=env('ADMIN_PANEL_SLUG'); ?>/newsletter/<?php echo $action; ?><?=!empty($email['id']) ? '/' . $email['id'] : '' ; ?>" method="post">
