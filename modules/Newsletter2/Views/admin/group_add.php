<div class="main-cont">
    ...
    <div class="c">
        <div class="head">
            <?php if(!empty($slider)): ?>
                <?=lang('Newsletter.GroupEdit'); ?>
            <?php else: ?>
                <?=lang('Newsletter.NewGroupAdd'); ?>
            <?php endif; ?>
        </div>
        <?= view('admin/alert_box', array('flashdata'=>!empty($flashdata) ? $flashdata : array())); ?>
        <form class="form slider-form" action="<?=$locale ? '/' . $locale : ''; ?>/<?=env('ADMIN_PANEL_SLUG'); ?>/newsletter/<?php echo $action; ?><?=!empty($group['id']) ? '/' . $group['id'] : '' ; ?>" method="post">
