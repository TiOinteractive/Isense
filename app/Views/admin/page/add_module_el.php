<div class="module order-item" data-no="<?=isset($no) ? $no : 0; ?>">
    ...
    <?php endif; ?>
    <div class="delete">
        <a href="<?=$locale ? '/' . $locale : ''; ?>/<?= env('ADMIN_PANEL_SLUG'); ?>/page/delete-module-el<?=!empty($content) ? '/' . $content['id'] : ''; ?>" class="delete-module-element" title="<?=lang('Admin.page.Remove'); ?>" data-title="<?=lang('Admin.page.DeleteModule'); ?>" data-message="<?=lang('Admin.page.DeleteModuleConfirm'); ?>" data-btn-ok="<?=lang('Admin.page.Remove'); ?>" data-btn-cancel="<?=lang('Admin.page.Cancel'); ?>" data-btn-close="<?=lang('Admin.page.Close'); ?>"><i class="fa-regular fa-trash-can"></i></a>
    </div>
</div>