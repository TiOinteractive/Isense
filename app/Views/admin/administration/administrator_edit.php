<?php
if (session()->getFlashdata('msg-send')) {
    ?>
    <script>
        $(document).ready(function () {
            $.alert({
                type: 'green',
                boxWidth: '300px',
                title: '<?= lang('Admin.administrators.Editadmin'); ?>',
                content: '<?= session()->getFlashdata('msg-send'); ?>',
                useBootstrap: false
            });
        });
    </script>	
    ...
    <div class="main-cont ajax-cont">
        ...
        <div class="c">
            <div class="head">
                <?=$administrator['name']; ?>
                <span><?= lang('Admin.administrators.Editadmin'); ?></span>
            </div>
            <form class="form slider-form" action="<?=$locale ? '/' . $locale : ''; ?>/<?= env('ADMIN_PANEL_SLUG'); ?>/administrators/save/<?= $administrator['id']; ?>" method="post" id="FormAdmin">
                <div class="form-row">
                    ...
                    <div class="form-field">
                        <input type="text" autocomplete="off" name="name" value="<?php if(isset($administrator['name'])) {echo $administrator['name'];} ;?>" />
                            <?php if (isset($result['errors']) and isset($result['errors']['name'])): ?>
                ...
                <div class="form-row">
                    ...
                    <div class="form-field">
                        <input type="text" autocomplete="off" name="login" value="<?php if(isset($administrator['login'])) {echo $administrator['login'];} ;?>" />
                            <?php if (isset($result['errors']) and isset($result['errors']['login'])): ?>
                            <div class="alert alert-warning">
                            <?= $result['errors']['login']; ?>
                            </div>
    <?php endif; ?>
                ...
                <div class="form-row">
                    ...
                    <div class="form-field">
                        <input type="text" autocomplete="off" name="email" value="<?php if(isset($administrator['email'])) {echo $administrator['email'];} ;?>" />
                            <?php if (isset($result['errors']) and isset($result['errors']['email'])): ?>
                            <div class="alert alert-warning">
                            <?= $result['errors']['email']; ?>
                            </div>
    <?php endif; ?>
                ...
                <div class="form-row">
                    <div class="form-label">
                        <label><?= lang('Admin.administrators.NewPassword'); ?> (<?= lang('Admin.administrators.RequiredChar', [8]); ?>)</label>
                    </div>
                    <div class="form-field">
                        <input type="password" autocomplete="off" name="password" value="">
                            <?php if (isset($result['errors']) and isset($result['errors']['password'])): ?>
                            <div class="alert alert-warning">
    ...
    <?php endif; ?>
                ...
                <div class="form-row">
                    <div class="form-label">
                        <label><?= lang('Admin.administrators.RepeatNewPassword'); ?> (<?= lang('Admin.administrators.RequiredChar', [8]); ?>)</label>
                    </div>
                    <div class="form-field">
                        <input type="password" autocomplete="off" name="password2" value="">
                            <?php if (isset($result['errors']) and isset($result['errors']['password2'])): ?>
                            <div class="alert alert-warning">
    ...
    <?php endif; ?>
				  ...
				  <select name="role">
					...
					if(isset($roles)) {
                     foreach($roles as $role_ind=>$role_val) {
						...
						echo '<option value="'.$role_ind.'"'; if($administrator['role']==$role_ind) {echo ' selected="selected" '; }echo '>'.$role_val.'</option>'; 
					 }	 
                    }
					?>
				 </select>
                ...
                <div class="form-row submit">
                    <button type="submit"><?= lang('Admin.administrators.Editadmin'); ?></button>
            </div>
        </form>
    </div>
</div>
